...
void SendCanFrames(HAL_TIM_ActiveChannel timChannel) {
	...
	if (timChannel == HAL_TIM_ACTIVE_CHANNEL_1) {
		...
		if (HAL_CAN_AddTxMessage(&hcan, &TxHeaderBmsDataPack5, TxData,
				&Tx_mailbox) != HAL_OK)
			Error_Handler();
		...
		TxData[7] = checksum;
		HAL_GPIO_TogglePin(FAULT_LED_GPIO_Port, FAULT_LED_Pin);
		if (HAL_CAN_AddTxMessage(&hcan, &TxHeaderBmsDataPack3, TxData,
				&Tx_mailbox) != HAL_OK)
			Error_Handler();
		...
		checksum = 0x00; // TODO
		TxData[7] = checksum;
		if (HAL_CAN_AddTxMessage(&hcan, &TxHeaderBmsDataPack4, TxData,
				&Tx_mailbox) != HAL_OK)
			Error_Handler();
		...
		if (HAL_CAN_AddTxMessage(&hcan, &TxHeaderBmsDataPack0, TxData,
				&Tx_mailbox) != HAL_OK)
			Error_Handler();
		...
		checksum = 0x00; // TODO
		TxData[7] = checksum;
		if (HAL_CAN_AddTxMessage(&hcan, &TxHeaderBmsDataPack2, TxData,
				&Tx_mailbox) != HAL_OK)
			Error_Handler();
		...
		TxData[7] = checksum;
		HAL_GPIO_TogglePin(CUSTOM_LED_GPIO_Port, CUSTOM_LED_Pin);
		if (HAL_CAN_AddTxMessage(&hcan, &TxHeaderBmsDataPack1, TxData,
				&Tx_mailbox) != HAL_OK)
			Error_Handler();
	}
}
